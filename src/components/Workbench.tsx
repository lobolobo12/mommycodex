...
export default function Workbench(){
  ...
  useEffect(()=>{
    if(h.tab!=='preview')return;let cancelled=false;let timer:ReturnType<typeof setTimeout>;
  ...
  return <aside className="workbench card" aria-label="Project workbench">
    ...
    {!cwd?<p>Choose a project to use the workbench.</p>:h.tab==='memory'?<form className="memory-form" onSubmit={e=>{e.preventDefault();setSaving(true);void run(()=>harness.saveMemory(cwd,memory).then(()=>setUrl(memory.previewUrl))).finally(()=>setSaving(false));}}>
      ...
      {h.browser?.screenshot&&h.browser.url?<img ref={previewRef} src={h.browser.screenshot} alt="Interactive live project preview" tabIndex={0} draggable={false}
        onClick={e=>{e.currentTarget.focus();const b=e.currentTarget.getBoundingClientRect();act({action:'click',x:(e.clientX-b.left)/b.width*(h.browser?.width??1000),y:(e.clientY-b.top)/b.height*(h.browser?.height??650)});}}
        onKeyDown={e=>{if(e.key==='Tab'||e.key==='Escape')return;e.preventDefault();const key=e.key===' '?'Space':e.key;if(!held.current.has(key)){held.current.add(key);act({action:'keydown',key});}}}
        ...
        onWheel={e=>act({action:'scroll',x:e.deltaX,y:e.deltaY})}/>:<div className="workbench-empty"><Icon name="code" size={30}/><strong>Your app, right here</strong><p>Start its server, then open the local URL.<br/>Click the preview to play or type.</p></div>}
      </div>
